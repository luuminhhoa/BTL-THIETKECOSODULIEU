...
\begin{itemize}
    \item \textbf{Trục Doanh nghiệp (Merchant):} Đóng vai trò là gốc của cấu trúc đa khách thuê (Multi-tenant). Mọi dữ liệu về chiến dịch (Campaign), người dùng (User), cấu hình API (APICredential) đều được định tuyến và cô lập dựa trên khóa ngoại \texttt{MerchantID}.
    \item \textbf{Trục Sổ cái Giao dịch (Ledger):} Tập trung vào thực thể \texttt{CreditTransaction}. Đây là rốn vũ trụ ghi nhận mọi biến động điểm thưởng, liên kết chặt chẽ với người dùng cuối (\texttt{Customer}), chiến dịch (\texttt{Campaign}) và chuỗi khối Sui để phục vụ đối soát.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Thực thể User (Tài khoản nhân sự):}
    \begin{itemize}
        \item \texttt{UserID} (PK): Mã định danh người dùng hệ thống.
        \item \texttt{MerchantID} (FK, NULL): Thuộc doanh nghiệp nào (Nếu NULL là Admin tổng).
        \item \texttt{Email} (NOT NULL, UNIQUE): Email đăng nhập.
        \item \texttt{PasswordHash} (NOT NULL): Mật khẩu băm.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{EndDate} (NULL): Ngày kết thúc.
    ...
    \begin{itemize}
        ...
        \item \texttt{ActionType} (NOT NULL): Loại hành động (Mua hàng, Sinh nhật).
        \item \texttt{PointsMultiplier} (NOT NULL): Hệ số nhân điểm.
    \end{itemize}
    
    \item \textbf{Thực thể Customer (Khách hàng / Người dùng cuối):}
    \begin{itemize}
        \item \texttt{CustomerID} (PK): Mã định danh ví Web3 (zkLogin Address).
        \item \texttt{WalletAddress} (NOT NULL, UNIQUE): Địa chỉ ví Sui.
        \item \texttt{Email} (NULL): Email định danh Web2.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{AvailableBalance} (NOT NULL): Số dư khả dụng hiện tại.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{Amount} (NOT NULL): Số điểm biến động (+/-).
