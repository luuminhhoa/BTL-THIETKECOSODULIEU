...
\begin{itemize}
    ...
    \item \textbf{Quy tắc 3 (Ánh xạ mối kết hợp M:N):} Tạo một quan hệ trung gian cho mối kết hợp nhiều-nhiều (ví dụ quá trình ghi nhận giao dịch điểm). Đưa khóa của hai thực thể vào làm khóa ngoại. Khóa chính thường là tổ hợp của các khóa ngoại này cùng với thuộc tính định danh riêng (nếu có).
\end{itemize}

\subsection{Danh sách Lược đồ Quan hệ sơ bộ}
Dựa trên ERD của OctaPoint CaaS, áp dụng các quy tắc ánh xạ, ta thu được tập hợp 11 lược đồ quan hệ sơ bộ như sau. (Quy ước: Thuộc tính gạch chân là Khóa chính - PK, thuộc tính in nghiêng là Khóa ngoại - FK).
...
\begin{description}
    ...
    \item[\texttt{Campaign}] (\underline{CampaignID}, \textit{TenantID}, CampaignName, StartDate, EndDate, Status)
    \item[\texttt{Campaign\_Merchant}] (\underline{\textit{CampaignID}}, \underline{\textit{MerchantID}})
    ...
    \item[\texttt{Credit}] (\underline{CreditID}, \textit{CustomerID}, \textit{TenantID}, AvailableBalance)
    \item[\texttt{CreditTransaction}] (\underline{TransactionID}, \textit{CreditID}, \textit{CampaignID}, \textit{MerchantID}, Amount, TransactionType, SuiTxDigest, CreatedAt)
